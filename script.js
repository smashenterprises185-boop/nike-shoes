...
// RENDER CURRENT SNEAKER IN SHOWCASE
...
// COLLECTIONS PAGE FILTER & RENDER
function filterCollection(category, btnEl) {
  ...
  const filtered = activeCollectionCategory === 'all' 
    ? SNEAKERS 
    : SNEAKERS.filter(s => s.category.toLowerCase().includes(activeCollectionCategory.toLowerCase()) || activeCollectionCategory === 'all');
...
function handleContactSubmit(e) {
  ...
  setTimeout(() => {
    ...
    if (alertEl) {
      alertEl.textContent = '✓ Thank you! Your inquiry has been dispatched to Nike Concierge. We will reply shortly.';
